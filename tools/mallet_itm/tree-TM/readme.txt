...
(5) Train tree-based topic model:
...
* Notice: if you use the whole wordnet to generate the tree ("Use All of WordNet" option in wordnet_readme.txt), when run step(5), add "--tree-model-type naive" at the end of the command of step (5), try both with and without this option, and check which one work faster for you, and use the faster one. (running with or without this option both correct).
