...
class MalletItmAnalysis(AbstractAnalysis):
    ...
    def set_constraints(self, merge_links, split_links):
        ...
        cmd = ['java', '-cp', 'class:lib/*', self.tree_class, '--vocab',
    ...
    def run_analysis(self, documents):
        ...
        if not os.path.exists(self.itm_vocab_file):
            cmd = ['java', '-cp', 'class:lib/*', self.vocab_class, '--input', self.mallet_imported_data_file, '--vocab', self.itm_vocab_file]
            print(' '.join(cmd))
            try:
                subprocess.check_call(cmd)
            except:
...
if __name__ == "__main__":
    corpus = [StubDocument("dog dog cat cat cat dog cat dog bark"),
              StubDocument("tree elm oak oak elm elm tree tree bark")]
    corpus = StubCorpus(corpus)

    mallet_location = "tree-TM/bin/mallet"
    ...
    itm = MalletItmAnalysis(mallet_location, ".",
