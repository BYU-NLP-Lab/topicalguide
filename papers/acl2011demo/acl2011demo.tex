...
% I also think that we must briefly address the issue of noise in the topic mark-up at the token level.  We need to at least acknowledge that topic assignments to tokens are not always meaningful, for the reasons we discussed yesterday:
% -          Distributional patterns found by inference are not necessarily semantically informative to humans.
% -          The topic assignments represent a sample from MCMC and are random, according to the state of the model at the time of the sample.
% -          (are there other reasons to mention?)
% We must also claim that despite the low-level noise, higher-level patterns are nonetheless meaningful.  How do we justify this claim in the absence of user studies?  Perhaps we simply highlight the need for user studies, paving the way for our future work, which we can mention.  We should acknowledge existing efforts in this direction, in particular the tea-leaves experiment of Boyd-Graber and others.
% --Eric
%p.s. I think this discussion belongs at the end of section 2 after “Or click on “Harry S. Truman 1.txt” to see President Truman’s message with words belonging to the “programs federal” topic highlighted Figure 3).”


%TODO Do we need to anonymize images by removing the logo, and the text by removing the URL?
...
%TODO -abstract, sec1, and/or sec9: I think we need to say something about how when you first encounter a large corpus, because you cannot read it, you may not even know what the predominant themes are.  Topic models expose the dominant themes, and the Topical Guide makes it possible to explore what the topic analysis has revealed.
...
\section{Document Metadata}%TODO Another suggestion: I think sections 3 and 5 should be merged.  Meta-data leads to plots, which can be discussed immediately rather than with a forward pointer.  The new merged section could go either before or after the current section 4 on metrics.
...
introduction \cite{Griffiths2004}. In our system they can be generated over any attribute for any%TODO Sec. 5: citation of Griffiths and Steyvers (2004) for plots should be preceded with an “e.g.”, since there were others who did similar things around that time.
...
are omitted to reduce visual complexity. However, the distances between nodes %TODO -sec6: In the final rendering of the image, edges are omitted to reduce visual complexity. -- perhaps the first order edges from the center topic could be reintroduced, with second order edges still omitted; when I look at the graphs without edges, it seems that something is missing.
...
\begin{figure*}[ht]%TODO % figure 5 could be improved to reflect both the procedural steps involved and the products of those steps.  I also think that figure 5 deserves a little more narration.
 \centering
...
All stages in the import process shown here can be overridden, allowing clean, domain-specific customizations.}
...
In order to make workflows simpler and more efficient, we implemented%TODO Sec. 8: “we implemented the import process as a command-line frontend”.  The section is titled “backend”.  Let’s go with “backend” or just “command-line process”.
...
the \tool{} will automatically invoke MALLET's LDA implementation to train a%TODO Sec. 8: “the Topical Guide will automatically invoke MALLET’s LDA implementation to train a topic model” --> “the Topical Guide import process will automatically invoke …”
