...
from topic_modeling.visualize.models import Topic, TopicName, Dataset
from topic_modeling.visualize.models import TopicGroup
from topic_modeling.visualize.models import TopicGroupTopic
from topic_modeling.visualize.models import TopicMetric, TopicMetricValue
from topic_modeling.visualize.models import PairwiseTopicMetric, PairwiseTopicMetricValue
...
def get_topic_infos(aid):
    from django.db import connection
    c = connection.cursor()
    c.execute('''select topic_id, document_id, d.filename, count(*) dc 
        from visualize_wordtoken_topics 
        join visualize_wordtoken wt on wt.id == wordtoken_id 
        join visualize_document d on d.id == wt.document_id 
        join visualize_topic t on t.id == topic_id where t.analysis_id == 1
        group by wt.document_id, topic_id order by topic_id, -dc''')
    topics = c.fetchall()
"""
